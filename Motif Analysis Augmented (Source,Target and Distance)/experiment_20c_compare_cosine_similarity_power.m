%{
    ...
    Modified by Danielle Nadin 2020-03-01
    Changed to compute cosine similarity of power instead of motifs
...
%% Setup the variables
power_folder = 'D:\DOC\Motif analysis\Results\Alpha\power';
...
%% loading the data
data = load(strcat(power_folder,filesep,ppt_name_1,filesep,ppt_state_1,'_power_td.mat'));
ppt_data_1 = data.result_td;

data = load(strcat(power_folder,filesep,ppt_name_2,filesep,ppt_state_2,'_power_td.mat'));
ppt_data_2 = data.result_td;
...
vector_1 = normalize(ppt_data_1.data.filt_power);
vector_2 = normalize(ppt_data_2.data.filt_power);

% Get the anterior data points
vector_1_anterior = get_anterior(vector_1, ppt_data_1.data.filt_location); %use the patient channel locations (< 99 chans)
vector_2_anterior = get_anterior(vector_2, ppt_data_1.data.filt_location);

% Get the posterior data points
vector_1_posterior = get_posterior(vector_1, ppt_data_1.data.filt_location);
vector_2_posterior = get_posterior(vector_2, ppt_data_1.data.filt_location);
...
function [ppt_data_1, ppt_data_2] = equalize(ppt_data_1, ppt_data_2)
    ...
    if (length(ppt_data_1.data.filt_location) == length(ppt_data_2.data.filt_location))
       return 
    end
    

    index_to_remove = [];
    for p1_i = 1:length(ppt_data_1.data.filt_location)
       label_1 = ppt_data_1.data.filt_location(p1_i).labels;

       is_found = 0;
       for p2_i = 1:length(ppt_data_2.data.filt_location)
          label_2 = ppt_data_2.data.filt_location(p2_i).labels;
    ...
    ppt_data_1.data.filt_power(index_to_remove) = [];
    ...
    ppt_data_2.data.filt_power(index_to_remove) = [];
end
