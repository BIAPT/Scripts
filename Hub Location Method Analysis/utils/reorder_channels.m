...
%EEG_MANIPULATION is a wrapper function to call the python function
    ...
    [num_location,labels] = get_num_location(wpli.location, channel_order);
...
function [num_location, labels] = get_num_location(location, total_location)
    num_location = 0;
    labels = {};
    for i = 1:length(total_location)
       label = total_location(i,1);
       
       if(get_index_label(location, label))
