...
data=pd.read_pickle('final_wpli_all_NEW.pickle')
...
plt.subplot(312)
plt.plot(np.mean(data12_anes.iloc[:,5:],axis=1))
plt.hlines(np.mean(np.mean(data12_anes.iloc[:,5:])),0,290)
plt.ylabel('wPLI')
plt.title('WSAS_12 during Anesthesia')


plt.subplot(313)
plt.plot(np.mean(data12_reco.iloc[:,5:],axis=1))
plt.hlines(np.mean(np.mean(data12_reco.iloc[:,5:])),0,290)
plt.xlabel('timestep')
plt.ylabel('wPLI')
